...
def callpayoff(Srange, K, premium=0): #payoff of a long call at expiration
    return np.maximum(Srange-K, 0)-premium

def putpayoff(Srange, K, premium=0): #payoff of a long put at expiration
    return np.maximum(K-Srange, 0)-premium

def coveredcall(Srange, S0, K, premium): #own the stock and sell a call against it, caps upside but you keep the premium
    stockpnl=Srange-S0
    shortcallpnl=-callpayoff(Srange, K, premium=0)+premium
    return stockpnl+shortcallpnl

def straddle(Srange, K, callprem, putprem): #long call and long put at the same strike, bets on a big move either direction
    return callpayoff(Srange, K, callprem)+putpayoff(Srange, K, putprem)

def ironcondor(Srange, Kputlong, Kputshort, Kcallshort, Kcalllong, netcredit): #sells a call spread and put spread together, profits if stock stays in a range
...
def butterfly(Srange, Klow, Kmid, Khigh, netdebit): #bets the stock lands near Kmid, sell 2 in the middle, buy 1 on each side
...
if __name__ == '__main__':
    Srange=np.linspace(50, 150, 200) #test plot: checking the basic long call payoff looks right

...
with st.expander("What is Black-Scholes actually doing?"):
    ...
    discounted back to today), weighted by the probability each of those things actually happens.

    It assumes the stock price moves randomly and can't go negative, with returns that are roughly

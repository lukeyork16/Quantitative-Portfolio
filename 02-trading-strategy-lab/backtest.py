...
def backtest(pricedata, signal): #works for any strategy. it just needs price data and a signal series (1,0,-1)
    dailyreturn=pricedata.pct_change()
    strategyreturn=signal.shift(1)*dailyreturn #this makes it shift so we trade on yesterday's signal and removes lookahead
    return strategyreturn

def backtestpairs(data1, data2, signal): #the backtest for pairs trading so signal applies long to stock1 and short to stock2
    return1=data1.pct_change()
    return2=data2.pct_change()
    strategyreturn=signal.shift(1)*return1-signal.shift(1)*return2 #signal 1 =longstock1/shortstock2,-1 flips it
    return strategyreturn
